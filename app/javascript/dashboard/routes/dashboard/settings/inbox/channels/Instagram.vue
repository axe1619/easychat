  ...
  mounted() {
    ...
    if (errorMessage) {
      ...
      if (errorCode === '400') {
        ...
        this.errorStateDescription = this.$t('INBOX_MGMT.ADD.INSTAGRAM.ERROR_AUTH');
      } else {
        this.errorStateMessage = this.$t('INBOX_MGMT.ADD.INSTAGRAM.ERROR_MESSAGE');
        this.errorStateDescription = errorMessage;
      }
    }
    // Clean query params so the error doesn't persist after reload
  ...
  methods: {
    async requestAuthorization() {
      ...
      try {
        ...
        this.errorStateMessage = this.$t('INBOX_MGMT.ADD.INSTAGRAM.ERROR_MESSAGE');
        this.errorStateDescription = error.message;
        this.hasError = true;
      } finally {
